...
def usage(status):
	print '''Usage: {} -u USER
	-c NUMCOMMENTS    The number of user comments to analyze
	-u USER	          The reddit user you wish to look up
	-p NUMPOSTS       The number of user posts to analyze
	-tb NUMTOP/BOT    The number of top and bottom comment/post scores'''.format(
...
if __name__ == '__main__':
	...
	b = nsmallest(NUMTOPBOT, enumerate(COMMENTHEAP), key=lambda x: x[1])
	print_heap(b)
	...
	b = nsmallest(NUMTOPBOT, enumerate(POSTHEAP), key=lambda x: x[1])
	print_heap(b)
